/*!
 * Metro 4 Components Library v4.1.16 build 648 (https://metroui.org.ua)
...
// Source: js/metro.v4.js
    ...
    if (window.METRO_HOTKEYS_FILTER_CONTENT_EDITABLE === undefined) {
        window.METRO_HOTKEYS_FILTER_CONTENT_EDITABLE = true;
    }
    if (window.METRO_HOTKEYS_FILTER_INPUT_ACCEPTING_ELEMENTS === undefined) {
        window.METRO_HOTKEYS_FILTER_INPUT_ACCEPTING_ELEMENTS = true;
    }
    if (window.METRO_HOTKEYS_FILTER_TEXT_INPUTS === undefined) {
        window.METRO_HOTKEYS_FILTER_TEXT_INPUTS = true;
    }
    if (window.METRO_HOTKEYS_BUBBLE_UP === undefined) {
        window.METRO_HOTKEYS_BUBBLE_UP = false;
    }
    if (window.METRO_THROWS === undefined) {
        window.METRO_THROWS = true;
    }
        ...
        changeAttribute: function (attributeName) {
            switch (attributeName) {
                ...
                case 'data-locale':
                    ...
                    break;
                case 'data-special':
                    this.changeAttrSpecial();
                    break;
                case 'data-exclude':
                    this.changeAttrExclude();
                    break;
            }
        }
    };
...
// Source: js/plugins/carousel.js
    var Carousel = {
        ...
        _effect: function (current, next, effect, to, interval) {
            ...
            switch (effect) {
                case 'slide':
                    Animation[func](current, next, duration, effectFunc);
                    break;
                case 'slide-v':
                    Animation[func](current, next, duration, effectFunc);
                    break;
                case 'fade':
                    Animation['fade'](current, next, duration, effectFunc);
                    break;
                default:
                    Animation['switch'](current, next);
            }
    ...
    var Input = {
        ...
        _create: function () {
            ...
            if (element.attr('type') === 'password' && o.revealButton !== false) {
                ...
                revealButton
                    .on(Metro.events.start, function () {
                        element.attr('type', 'text');
                    })
                    .on(Metro.events.stop, function () {
            ...
            if (o.backend !== "") {
                var backend = Utils.isTag(o.backend) ? $(o.backend) : $("<span>" + o.backend + "</span>");
                backend.addClass("backend").addClass(o.clsBackend).appendTo(container);
            }
...
// Source: js/plugins/slider.js
    var Slider = {
        ...
        _convert: function (v, how) {
            ...
            switch (how) {
                case "pix2prc":
                    return Math.round(v * 100 / length);
                case "pix2val":
                    return Math.round(this._convert(v, 'pix2prc') * ((o.max - o.min) / 100) + o.min);
                case "val2prc":
                    return Math.round((v - o.min) / ((o.max - o.min) / 100));
                case "prc2pix":
                    return Math.round(v / (100 / length));
                case "val2pix":
                    return Math.round(this._convert(this._convert(v, 'val2prc'), 'prc2pix'));
            }
        },
...
// Source: js/plugins/validator.js
    var ValidatorFuncs = {
        ...
        validate: function (el, result, cb_ok, cb_error) {
            ...
            var funcs = input.data('validate') !== undefined ? String(input.data('validate')).split(" ").map(function (s) {
                return s.trim();
            }) : [];
            var errors = [];
    ...
    var Window = {
        ...
        changeAttribute: function (attributeName) {
            switch (attributeName) {
                ...
                case "data-btn-min":
                case "data-btn-max":
                    this.toggleButtons(attributeName);
                    break;
                case "data-width":
                case "data-height":
                    this.changeSize(attributeName);
                    break;
                case "data-cls-window":
                case "data-cls-caption":
                case "data-cls-content":
                    this.changeClass(attributeName);
                    break;
                case "data-shadow":
                    this.toggleShadow();
                    break;
                ...
                case "data-top":
                case "data-left":
                    this.changeTopLeft(attributeName);
                    break;
                case "data-place":
                    this.changePlace();
                    break;
